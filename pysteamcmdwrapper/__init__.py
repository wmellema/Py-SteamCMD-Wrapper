...
class SteamCMD():
    ...
    def _prepare_installation(self):
        ...
        self.exe = os.path.join(self._installation_path,"steamcmd"+package_links[self.platform]["extention"])
    ...
    def install(self, force=False):
        ...
        try:
            ...
            if e.returncode == 7:
                self._print_log("SteamCMD has returned error code 7 on fresh installation.",
                "",
                "Not sure why this crashed, long live steamcmd and it's non existent documentation...",
    ...
    def workshop_update(self, app_id, workshop_id, install_dir = None, validate = None, n_tries = 5):
        ...
        try:
            ...
            if e.returncode == 10 and _validate:
                ...
                raise SteamCMDDownloadException("SteamCMD was not able to complete this download due to a download timeout. Please add the Validate flag in order to keep retrying this download.")
